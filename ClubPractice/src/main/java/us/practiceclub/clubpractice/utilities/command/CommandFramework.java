package us.practiceclub.clubpractice.utilities.command;
...
public class CommandFramework implements CommandExecutor {
        ...
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (!command.permission().equalsIgnoreCase("") && !sender.hasPermission(command.permission())) {
                    sender.sendMessage(C.colorize(command.noPermission()));

                    if (command.inGameOnly()) {
                        Player player = (Player) sender;
                        player.playSound(player.getLocation(), Sound.WOLF_BARK, 20f, 20f);
                    }
    ...
    public void registerCompleter(String label, Method m, Object obj) {
        ...
        } else if (map.getCommand(cmdLabel) != null) {
            try {
